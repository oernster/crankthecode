...
| HTML is never cached anywhere: browser, proxy or CDN. | `test_html_cache_headers_are_no_store` in `tests/test_static_delivery.py` |
| Fingerprinted static assets are cached immutably for a year; a favicon never is. | `test_fingerprinted_static_assets_are_immutable_cached`, `test_favicon_is_not_cached_forever` in `tests/test_static_delivery.py`; `test_caching_staticfiles_cache_headers` in `tests/test_assets_caching.py` |
| The app starts and serves on a checkout where the static build has never run. | `test_app_starts_when_the_static_build_has_not_run` in `tests/test_static_delivery.py` |
| `CTC_STATIC_DIST_DIR` selects the served directory, not merely the CV lookup. | `test_cv_pdf_prefers_static_dist_when_enabled` and `test_app_starts_when_the_static_build_has_not_run` in `tests/test_static_delivery.py` |
| The CV is reachable at a stable root URL regardless of which static directory is mounted. | `test_cv_pdf_is_served_from_stable_root_path` in `tests/test_static_delivery.py` |
| EPUB files stay in the repository and are never downloadable, whether or not `docs/` exists. | `test_docs_epub_is_not_served`, `test_docs_directory_can_be_present_without_exposing_epub_downloads` in `tests/test_static_delivery.py` |
...
| The sitemap lists the main pages, the posts, the essays and the build log, never the retired pages; robots.txt always names the sitemap. | `test_sitemap_lists_main_pages_and_posts`, `test_robots_txt_includes_sitemap_location` in `tests/test_seo.py`, `test_sitemap_includes_essays_and_build_log_but_no_retired_pages` in `tests/test_topic_hubs.py` |
...
| A post with malformed frontmatter degrades the page instead of failing the request. | `test_load_about_html_fail_open_for_missing_post_and_exception` in `tests/test_html_router_coverage.py`, `test_homepage_leadership_missing_posts_is_tolerated` in `tests/test_homepage_leadership.py` |
